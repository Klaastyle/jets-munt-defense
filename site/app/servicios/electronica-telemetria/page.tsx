...
export default function ElectronicsTelemetryPage() {
  const interfaces: Array<{title: string, desc: string, icon: TechnicalIconName}> = [
    {
      title: "CAN Bus",
  ...
  const management: Array<{title: string, desc: string, icon: TechnicalIconName}> = [
    { title: "Telemetría en Tiempo Real", desc: "Monitorización constante de revoluciones, temperatura de escape (EGT), estado de bombas y consumo.", icon: "chart-line" },
    { title: "Fusión de Sensores Inteligente", desc: "Algoritmos de filtrado Kalman integrados en el firmware para una lectura de sensores ultraestable.", icon: "kalman-filter" },
    { title: "Compensación Ambiental", desc: "Ajuste automático del mapa de inyección basándose en presión atmosférica, humedad y temperatura.", icon: "environment-sensor" },
    { title: "Funciones de Protección", desc: "Apagado automático preventivo ante anomalías de llama, sobrecorrientes o pérdidas de señal.", icon: "shield-check" },
    { title: "Control de RPM de Precisión", desc: "Control orientado al campo (FOC) con respuesta instantánea del acelerador y empuje predecible.", icon: "sine-wave" },
    { title: "Monitoreo de Salud", desc: "Algoritmo predictivo de desgaste mecánico y fatiga de componentes basado en ciclos térmicos.", icon: "monitor-settings" },
  ];
      ...
      <section className="section container" style={{ paddingBottom: "3rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "3rem", marginBottom: "4rem" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gap: "1.2rem" }}>
              {interfaces.map((inf) => (
                <div key={inf.title} style={{ display: "flex", gap: "1rem", alignItems: "flex-start", background: "rgba(255,255,255,0.02)", padding: "1rem", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ color: "var(--accent)", padding: "0.2rem" }}>
                    <TechnicalIcon name={inf.icon} size={28} />
                  </div>
                  <div>
        ...
        <div style={{ background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "6px", padding: "2.5rem", marginBottom: "4rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "2rem" }}>
            {management.map((mng) => (
              <div key={mng.title} style={{ display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                <div style={{ color: "var(--text-muted)", padding: "0.1rem" }}>
                  <TechnicalIcon name={mng.icon} size={26} strokeWidth={1.5} />
                </div>
                <div>
